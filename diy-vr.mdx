...
The easy way would have been to use VRidge on another phone that has a gyroscope sensor, or just get an actual VR controller, or even build my own controllers using the same method as the headset. But I couldn't do any of those things.
...
- Because I used the MPU6050 gyroscope sensor, it only read my head's rotations, not movements, so if I were to move my head forwards it wont do so in game, unlike proper desktop VR headsets.
